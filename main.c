...
/*   Updated: 2023/11/29 19:04:24 by aldokezer        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char *argv[], char *envp[])
{
	...
	while (ft_process(&process, ft_no_of_commands(argc, argv)))
	{
		...
		if (pid == 0)
		{
			tst_ft_redir_pipes(io_fd, process, pipe_fd, ft_no_of_commands(argc, argv));
			tst_ft_cls_fd(pipe_fd, io_fd, ft_no_of_commands(argc, argv));
